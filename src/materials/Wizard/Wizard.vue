      ...
      <md-card class="md-card-wizard active" data-color="green">
        ...
        <md-card-actions md-alignment="space-between">
          <slot name="footer" :next-tab="nextTab" :prev-tab="prevTab">
            <div>
              <md-button v-if="activeTabIndex > 0" @click.native="prevTab" class="btn-previous">{{ prevButtonText }}</md-button>
            </div>

            <div>
              <md-button v-if="activeTabIndex < tabCount - 1" @click.native="nextTab" class="btn-next md-success">{{ nextButtonText }}</md-button>
              <md-button v-else class="md-round md-success md-sm" @click.native="nextTab">{{finishButtonText}}</md-button>
            </div>
          </slot>
        </md-card-actions>
      </md-card>
